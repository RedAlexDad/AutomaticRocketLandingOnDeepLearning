...
class Rocket(QWidget):
    def __init__(self, terrain_generator, parent=None):
        ...
        self.x = self.terrain_generator.width() // 2 - self.terrain_generator.width() // 14
        # Коррекция координаты Y
        # self.y = int(self.terrain_generator.get_terrain_height(self.x))
        self.y = self.terrain_generator.height() // 2
    ...
    def paintEvent(self, event):
        ...
        triangle = QPolygonF()
        triangle.append(QPointF(self.x, self.height() - self.y))  # Изменение направления по оси Y
        triangle.append(QPointF(self.x - 5, self.height() - (self.y - 10)))  # Изменение направления по оси Y
        triangle.append(QPointF(self.x + 5, self.height() - (self.y - 10)))  # Изменение направления по оси Y
    ...
    def move_rocket(self, delta_x, delta_y):
        ...
        print('get_terrain_height(new_x):', self.terrain_generator.get_terrain_height(new_x))
...
class TerrainGenerator(QWidget):
    ...
    def generate_terrain_function(self):
        # Пример: Синусоидальная функция с некоторым шумом
    ...
    def generate_terrain(self):
        ...
        for i in range(self.num_points):
            ...
            y = int(self.height() - self.get_terrain_height(x))
...
class CoordinateGrid(QWidget):
    ...
    def paintEvent(self, event):
        ...
        for x in range(0, self.width(), self.grid_size):
            ...
            painter.drawText(x, self.height() - 5, str(x))  # Метка для оси X

        for y in range(0, self.height(), self.grid_size):
            painter.drawLine(0, y, self.width(), y)
            painter.drawText(0, self.height() - y, str(y))  # Метка для оси Y

        # Добавление надписей к осям координат
        ...
        # Надпись по оси X
        painter.drawText(self.width() - 20, self.height() - 20, "X")
...
class MainWindow(QWidget):
    def __init__(self, width=400, height=400, cell_move_speed=5):
        super().__init__()
        ...
        self.coordinates_label = QLabel("Rocket: (0, 0)")
        # Создание ракеты
        ...
        self.cell_move_speed = cell_move_speed
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == Qt.Key_Left:
            self.rocket.move_rocket(-self.cell_move_speed, 0)  # Перемещение ракеты влево
        elif event.key() == Qt.Key_Right:
            self.rocket.move_rocket(self.cell_move_speed, 0)  # Перемещение ракеты вправо
        elif event.key() == Qt.Key_Up:
            self.rocket.move_rocket(0, -self.cell_move_speed)  # Перемещение ракеты вверх
        elif event.key() == Qt.Key_Down:
            self.rocket.move_rocket(0, self.cell_move_speed)  # Перемещение ракеты вниз
