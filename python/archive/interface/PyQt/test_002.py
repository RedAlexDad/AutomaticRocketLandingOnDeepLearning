...
class RocketLanderApp(QMainWindow):
    ...
    def draw_terrain(self):
        ...
        for i in range(1, len(self.terrain)):
            x1 = (i - 1) * x_scale
            y1 = -int(self.height / 2) + int(self.terrain[i - 1] * y_scale)
            x2 = i * x_scale
            y2 = -int(self.height / 2) + int(self.terrain[i] * y_scale)
    ...
    def draw_rocket(self):
        rocket_start_x = 0 - self.rocket_width / 2
        rocket_start_y = -self.height / 2  # Устанавливаем ракету на нижнюю границу окна
    ...
    def move_rocket(self, event):
        ...
        y = -rocket_pos.y() - self.rocket_height / 2  # Инвертируем значение координаты Y

        if key == Qt.Key_Left and x > -self.width / 2:
            self.rocket_item.moveBy(-5, 0)
        elif key == Qt.Key_Right and x < self.width / 2 - self.rocket_width:
            self.rocket_item.moveBy(5, 0)
        elif key == Qt.Key_Up and y > -self.height / 2:
            self.rocket_item.moveBy(0, -5)
        elif key == Qt.Key_Down and y < self.height / 2 - self.rocket_height:
            self.rocket_item.moveBy(0, 5)
