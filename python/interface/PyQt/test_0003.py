...
class CoordinateGrid(QWidget):
    def __init__(self, parent=None):
        ...
        self.rocket_coordinates = QPointF(0, 0)
    ...
    def paintEvent(self, event):
        ...
        # Отображение координат ракеты
        painter.drawText(self.rocket_coordinates,
                         f"Rocket: ({self.rocket_coordinates.x()}, {self.rocket_coordinates.y()})")

    def resizeEvent(self, event):
        # Обновление размеров и позиции метки ракеты при изменении размера виджета
        self.rocket_label.move(self.width() - 100, 10)

    def set_rocket_coordinates(self, x, y):
        # Установка координат ракеты и обновление отображения
        self.rocket_coordinates = QPointF(x, y)
        self.update()
