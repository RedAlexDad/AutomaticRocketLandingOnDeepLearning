...
class RocketLanderApp(QMainWindow):
    ...
    def initUI(self):
        ...
        self.setGeometry(100, 100, self.width + 100, self.height + 100)
    ...
    def draw_rocket(self):
        ...
        rocket_start_y = self.rocket_height
    ...
    def move_rocket(self, event):
        key = event.key()
        rocket_item = self.scene.items()[0]
        rocket_coords = rocket_item.polygon()
        x = (rocket_coords.at(0).x() + rocket_coords.at(1).x() + rocket_coords.at(2).x()) / 3
        y = self.height - (rocket_coords.at(0).y() + rocket_coords.at(1).y() + rocket_coords.at(2).y()) / 3

        if key == Qt.Key_Left and x > 0:
            rocket_item.moveBy(-5, 0)
        elif key == Qt.Key_Right and x < self.width - self.rocket_width:
            rocket_item.moveBy(5, 0)
        elif key == Qt.Key_Up and y > 0:
            rocket_item.moveBy(0, -5)
...
def main():
    ...
    window = RocketLanderApp(100, 0.1, width=1200, height=600)
    window.show()
    sys.exit(app.exec_())
